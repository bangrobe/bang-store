...
import { mockProducts, mockDailyRevenue } from "@/lib/mock";
...
export default function ReportsPage() {
  ...
  const filteredRevenue = useMemo(() => {
    return mockDailyRevenue.filter(
      (d) => d.date >= dateFrom && d.date <= dateTo
    );
  }, [dateFrom, dateTo]);

  const totalRevenue = useMemo(
    () => filteredRevenue.reduce((s, d) => s + d.revenue, 0),
    [filteredRevenue]
  );

  const totalOrders = useMemo(
    () => filteredRevenue.reduce((s, d) => s + d.orders, 0),
    [filteredRevenue]
  );
  ...
  const maxRevenue = Math.max(
    ...filteredRevenue.map((d) => d.revenue),
    1
  );
